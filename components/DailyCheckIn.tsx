...
export default function DailyCheckIn() {
    ...
    return (
        <div className="w-full min-h-screen flex flex-col items-center justify-center bg-[#FAF8F5] relative overflow-hidden p-6 font-sans">

            {/* Top Left Geometric Corner (Inverted L) - Thinner and sharper */}
            <div className="absolute top-8 left-8 w-6 h-6 pointer-events-none">
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {step === "intro" && (
                            <motion.div key="intro" {...fadeInUp} className="w-full flex flex-col items-center text-center space-y-20">
                                ...
                                <button
                                    onClick={handleNext}
                                    className="w-full max-w-[280px] py-4 border border-[#2A2A2A] text-[#E07A5F] text-lg font-bold hover:bg-[#2A2A2A] hover:text-[#FAF8F5] transition-colors"
                                >
                                    시작하기
                                </button>
                            </motion.div>
                        ...
                        {step === "name" && (
                            <motion.div key="name" {...fadeInUp} className="w-full flex flex-col items-center text-center space-y-12">
                                <h3 className="text-[#2A2A2A] text-lg font-bold leading-relaxed mt-4">
                                    당신을 무엇이라고<br />
                                    부르면 좋을까요?
                                </h3>
                                ...
                                <div className="w-full max-w-[320px] space-y-8">
                                    <input
                                        ...
                                        placeholder="입력해주세요"
                                        className="w-full py-4 border border-[#2A2A2A] bg-transparent text-center text-[#2A2A2A] placeholder:text-[#E07A5F]/40 outline-none focus:bg-white/40 transition-colors font-medium text-lg"
                                    ...
                                    {profile.name && (
                                        <button
                                            onClick={handleNext}
                                            className="w-full py-4 border border-[#2A2A2A] text-[#E07A5F] font-bold hover:bg-[#2A2A2A] hover:text-[#FAF8F5] transition-colors"
                                        >
                                            다음
                                        </button>
                                    )}
                        ...
                        {/* Birth Step */}
                        {step === "birth" && (
                            <motion.div key="birth" {...fadeInUp} className="w-full flex flex-col items-center text-center space-y-10">
                                <h3 className="text-[#2A2A2A] text-lg font-bold leading-relaxed mt-4">
                                    당신은 언제 이 세상에<br />
                                    도착했나요?
                                </h3>
                                ...
                                <div className="w-full max-w-[320px] flex flex-col gap-4">
                                    ...
                                    <div className="w-full flex border border-[#2A2A2A] bg-transparent focus-within:bg-white/40 transition-colors">
                                        <input
                                            ...
                                            maxLength={2}
                                            inputMode="numeric"
                                            className="flex-1 py-4 text-center bg-transparent text-[#2A2A2A] placeholder:text-[#E07A5F]/40 outline-none font-medium text-lg tracking-widest"
                                        ...
                                        />
                                        <span className="py-4 text-[#2A2A2A]/20">|</span>
                                        <input
                                            id="day-part-input"
                                            type="text"
                                            placeholder="DD"
                        ...
                        {step === "feeling" && (
                            <motion.div key="feeling" {...fadeInUp} className="w-full flex flex-col items-center text-center space-y-12">
                                <h2 className="text-[#2A2A2A] text-lg font-bold leading-relaxed mt-4">
                        ...
                        {step === "loading" && (
                            <motion.div key="loading" {...fadeInUp} className="w-full flex flex-col items-center text-center space-y-8 mt-20">
                                ...
                                <p className="text-[#E07A5F] animate-pulse font-bold tracking-widest">
                                    별들의 문장을 해석하는 중...
                                </p>
                            </motion.div>
                        )}
